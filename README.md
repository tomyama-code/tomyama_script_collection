...
#### リストを返す式同士の演算が正しく評価されない
リストを生成する関数（`linstep` など）同士を演算子（`/` など）で結合した際、期待しない結果（不完全なリストなど）が返される不具合があります。

**再現例:**
```text
\$ ./c 'linstep( 5, -1, 3 ) / linstep( 3, -1, 3 )' -v
linstep( 5, -1, 3 ) = ( 5, 4, 3 )
linstep( 3, -1, 3 ) = ( 3, 2, 1 )
2 / 1 = 2
Formula: 'linstep( 5, -1, 3 ) / linstep( 3, -1, 3 ) ='
    RPN: '# 5 -1 3 linstep # 3 -1 3 linstep /'
 Result: ( 5, 4, 3, 3, 2 )
```

**技術的なメモ・今後の課題:**
- `/` を処理する時点で、左オペランド（`linstep`）が「リスト」と「スカラ」のどちらなのかを動的に判別する仕組みが必要。
- 演算子、もしくはリストに `#`（終端）の概念を導入する案を検討中。
