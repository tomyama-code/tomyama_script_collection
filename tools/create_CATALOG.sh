...
sh_main()
{
    ...
    echo ""                 >>"$targfile.new"
    echo "### VERSION"      >>"$targfile.new"
    echo "- This document was generated by $appname [$rev]." >>"$targfile.new"
    echo ""                 >>"$targfile.new"
...
parse_input()
{
    while [ 1 ]; do
        ...
        '-h' | '--help')
            ...
            echo ""
            echo "VERSION"
            echo '  This document describes $Revision: 1.20 $.'
            echo ""
            usage
            echo ""
            echo "DESCRIPTION"
...
sh_help2markdown()
{
    cat - | perl -MFile::Basename -ne '
        ...
        if( $line =~ m!([^\s]+\.md)!o ){
            my $base_doc_name = $1;
            my $base_doc_bname = File::Basename::basename( $base_doc_name );
            my $base_doc_link = q{[} . $base_doc_bname . q{](} . $base_doc_bname . q{)};
            $line =~ s![^\s]+\.md!$base_doc_link!go;
        }
